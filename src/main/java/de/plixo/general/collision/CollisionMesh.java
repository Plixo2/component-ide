...
public class CollisionMesh {
    ...
    public CollisionMesh(@NotNull List<Vertex> vertices, @NotNull List<Integer> indices) {
        ...
        for (int i = 0; i < indices.size(); i += 3) {
            val indexA = indices.get(i);
            val indexB = indices.get(i + 1);
            val indexC = indices.get(i + 2);
            val a = vertices.get(indexA);
            val b = vertices.get(indexB);
            val c = vertices.get(indexC);
